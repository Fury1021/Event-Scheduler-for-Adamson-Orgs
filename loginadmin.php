...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT adminid FROM $table_name WHERE email = '$email' AND password = '$password'";
    $result = $conn->query($sql);

    // Check if the login credentials are valid
    if ($result->num_rows > 0) {
        // Fetch the adminid from the result
...
.inputboxesadmin input[type="email"],
.inputboxesadmin input[type="password"] {
  width: 90%;
  padding: 10px;
  margin-bottom: 5px;
  border-radius: 5px;
...
.loginbuttonadmin input[type="submit"] {
  width: 100%;
  padding: 10px;
...
<body>
    ...
    <form method="post" action="loginadmin.php">
		...
		<div class="loginbuttonadmin">
        <input type="submit" value="Login">
		</div>
    </form>
	
	<div class="button1admin">
    <form action="createadmin.php" method="get">
        <button type="submit" class="button2-1admin">Sign Up</button>
    </form>
    <form action="forgotpassword.php" method="get">
        <button type="submit" class="button2-2admin">Forgot Password</button>
    </form>
	</div>
